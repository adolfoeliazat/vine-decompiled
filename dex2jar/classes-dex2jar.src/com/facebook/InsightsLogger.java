...
public class InsightsLogger
{
  ...
  private Session sessionToLogTo()
  {
    try
    {
      Session localSession = this.specifiedSession;
      if ((localSession == null) || (!localSession.isOpened()))
        localSession = Session.getActiveSession();
      if ((localSession == null) || (!localSession.isOpened()) || (localSession.getAccessToken() == null))
      {
        if (appAuthSession == null)
        {
          ...
          appAuthSession.open(localAccessToken, null);
        }
        localSession = appAuthSession;
      }
      return localSession;
    }
    finally
