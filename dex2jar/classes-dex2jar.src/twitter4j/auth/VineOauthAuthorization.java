  ...
  implements Authorization, Serializable, OAuthSupport
{
  ...
  public VineOauthAuthorization(Configuration paramConfiguration)
  {
  ...
  {
    if (this == paramObject);
    VineOauthAuthorization localVineOauthAuthorization;
    do
    {
      return true;
      if (!(paramObject instanceof OAuthSupport))
        return false;
      localVineOauthAuthorization = (VineOauthAuthorization)paramObject;
      if (this.consumerKey != null)
      {
        if (this.consumerKey.equals(localVineOauthAuthorization.consumerKey));
      }
      else
        while (localVineOauthAuthorization.consumerKey != null)
          return false;
      if (this.consumerSecret != null)
      {
        if (this.consumerSecret.equals(localVineOauthAuthorization.consumerSecret));
      }
      else
        while (localVineOauthAuthorization.consumerSecret != null)
          return false;
      if (this.oauthToken == null)
        break;
    }
    while (this.oauthToken.equals(localVineOauthAuthorization.oauthToken));
    while (true)
    {
      return false;
      if (localVineOauthAuthorization.oauthToken == null)
        break;
    }
  }
  ...
  String generateAuthorizationHeader(String paramString1, String paramString2, HttpParameter[] paramArrayOfHttpParameter, String paramString3, String paramString4, OAuthToken paramOAuthToken)
  {
    ...
    if (LOGGABLE)
      Log.v("t4jVineOauth", "OAuth base string: " + str1);
    String str2 = generateSignature(str1, paramOAuthToken);
    if (LOGGABLE)
      Log.v("t4jVineOauth", "OAuth signature: " + str2);
  ...
  String generateSignature(String paramString, OAuthToken paramOAuthToken)
  {
    ...
    {
      if (LOGGABLE)
        Log.v("t4jVineOauth", "Failed initialize \"Message Authentication Code\" (MAC)", localInvalidKeyException);
      throw new AssertionError(localInvalidKeyException);
    }
    catch (NoSuchAlgorithmException localNoSuchAlgorithmException)
    {
      if (LOGGABLE)
        Log.v("t4jVineOauth", "Failed to get HmacSHA1 \"Message Authentication Code\" (MAC)", localNoSuchAlgorithmException);
    ...
    throws TwitterException
  {
    try
    {
      ...
      HttpClientWrapper localHttpClientWrapper = http;
      HttpParameter[] arrayOfHttpParameter = new HttpParameter[4];
      ...
      arrayOfHttpParameter[3] = new HttpParameter("send_error_codes", true);
      this.oauthToken = new AccessToken(localHttpClientWrapper.post(str, arrayOfHttpParameter, this));
 ...
 * Qualified Name:     twitter4j.auth.VineOauthAuthorization
